...
export const createRewardValue = async (data: IRewardValue): Promise<IRewardValue> => {
  try {
...
export const redeemReward = async (
  ...
  try {
    if (!data.userId || !data.rewardId) {
      throw new ApiError(httpStatus.FAILED, "User ID and Reward ID are required");
    }
    
    // Fetch the reward
    const reward: IReward | null = await rewardModel.getRewardbyId(data.rewardId);
    ...
    const rewardValue = await rewardModel.getAvailableRewardValue(data.rewardId);
